...
/datum/status_effect/chem/SGDF/on_remove()
	...
	to_chat(owner, span_warning("Lucidity shoots to your previously blank mind as your mind suddenly finishes the cloning process. You marvel for a moment at yourself, as your mind subconsciously recollects all your memories up until the point when you cloned yourself. Curiously, you find that you memories are blank after you ingested the synthetic serum, leaving you to wonder where the other you is."))
	fermi_Clone = null
	return ..()
...
/datum/status_effect/chem/astral_insurance/tick(mob/living/carbon/M)
	...
	if(owner.mind)
		...
		to_chat(G, "<span class='warning'>[M]'s consciousness snaps back to them as their astrogen runs out, kicking your projected mind out!'</b></span>")
...
/datum/status_effect/chem/astral_insurance/on_remove() //God damnit get them home!
	if(owner.mind != originalmind) //If they're home, HOORAY
		if(owner.mind)
			...
			owner.mind.transfer_to(G)//Just in case someone else is inside of you, it makes them a ghost and should hopefully bring them home at the end.
			to_chat(G, "<span class='warning'>[owner]'s consciousness snaps back to them as their astrogen runs out, kicking your projected mind out!'</b></span>")
