...
/obj/structure/blacksmith/anvil/proc/HandleIngot(obj/item/blacksmith/ingot/workpiece, mob/user)
	...
	if(workpiece.workability == "shapeable")
		...
		if(workpiece_state == WORKPIECE_PRESENT)
			add_overlay(image(icon= 'modular_atom/blacksmith/icons/blacksmith.dmi',icon_state="workpiece"))
			set_light_on(TRUE)
...
/obj/structure/blacksmith/anvil/proc/do_shaping(mob/user, qualitychange)
	...
	switch(stepdone) // Have some more then
		if("weak hit")
			playsound(src, 'modular_atom/blacksmith/sound/anvil_weak.ogg',100)
		...
		if("fold")
			playsound(src, 'modular_atom/blacksmith/sound/anvil_double1.ogg',95)
			do_smithing_sparks(1, TRUE, src)
		...
		if("punch")
			playsound(src, 'modular_atom/blacksmith/sound/anvil_single2.ogg',95)
			do_smithing_sparks(1, TRUE, src)
