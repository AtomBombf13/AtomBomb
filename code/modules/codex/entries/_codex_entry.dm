...
/datum/codex_entry/New(_display_name, list/_associated_paths, list/_associated_strings, _lore_text, _mechanics_text, _antag_text)
	...
	if(length(associated_paths))
		for(var/tpath in associated_paths)
			...
			LAZYOR(associated_strings, thing_name)
	...
	LAZYOR(associated_strings, codex_sanitize(name))
...
/datum/codex_entry/proc/get_codex_body(mob/presenting_to, include_header = TRUE, include_footer = TRUE)
	...
	if(antag_text && (!presenting_to || (presenting_to.mind && !is_special_character(presenting_to))))
