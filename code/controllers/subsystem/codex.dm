SUBSYSTEM_DEF(codex)
	...
	var/datum/codex_category/categories/categorized_categories
...
/datum/controller/subsystem/codex/Initialize()
	...
	categorized_categories = new /datum/codex_category/categories

	//Categories
	for(var/cattype in (subtypesof(/datum/codex_category)-/datum/codex_category/categories))
		var/datum/codex_category/cattry = cattype
		if(initial(cattry.name))
			cattry = new cattry()
	...
	for(var/ctype in SScodex.categories)
		var/datum/codex_category/cat = ctype
...
/datum/controller/subsystem/codex/proc/present_codex_entry(mob/presenting_to, datum/codex_entry/entry)
	if(entry && istype(presenting_to) && presenting_to.client)
		var/datum/browser/popup = new(presenting_to, "codex\ref[entry]", "Survival Guide", nheight=425)
